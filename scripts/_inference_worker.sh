#!/usr/bin/env bash
# Per-node worker for run_inference_only_multinode.sh
# Launches: SGLang (GPUs 0-5, tp=2 dp=3), sampler shim, 2 env services.
# Args (env vars passed from caller):
#   TTT_ROOT, RUN_ROOT, CONFIG, MODEL_NAME
#   NODE_IDX (0 or 1 — used as sampler idx)
#   ENV_IDX_BASE (0 or 2 — env services idx start)
set -euo pipefail
...
# SGLang 0.5.9 hard-fails startup on PyTorch 2.9.1 + cuDNN < 9.15 because of
# a Conv3d perf bug. We're text-only (no vision) so this doesn't apply. The
# bypass is the env var SGLang itself documents in the error message.
export SGLANG_DISABLE_CUDNN_CHECK="${SGLANG_DISABLE_CUDNN_CHECK:-1}"

# Note: HF_HUB_OFFLINE=1 short-circuits snapshot_download (avoiding the NFS
# flock stale-handle race when many ranks contend), but it ALSO blocks
# transformers' optional metadata lookups inside SGLang ServerArgs init.
# Left unset by default; ensure the model snapshot is fully pre-cached
# (config.json + tokenizer.json + safetensors) before launching.
...
# ---- SGLang (GPUs 0-3, tp=2 dp=2, base model only — no LoRA) --------------
# Inference-only: skip --enable-lora entirely so we can use dp>1 (SGLang
# rejects dynamic LoRA loading when dp_size != 1). tp=2 satisfies the model's
# vocab divisibility (201088). dp=2 gives 2 parallel replicas per node and
# frees GPUs 4-5 for additional env services (eval throughput bottleneck).
echo "[worker:$NODE_HOST] launching SGLang (GPUs 0-3, tp=2 dp=2, base model) → $LOGS/sglang_${SUFFIX}.log"
CTX_LEN="${SGLANG_CONTEXT_LENGTH:-65536}"
...
SGLANG_PID=$!
echo "$SGLANG_PID" > "$RUN_ROOT/sglang_${SUFFIX}.pid"
